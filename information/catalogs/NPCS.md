...
- `gravecaller` is still factory-defined, but the current map content uses `vaelith_marr` as the Blackglass Well boss implementation.
...
| Hadrik | Named merchant | Hadrik's Provisions | Merchant and `Keep the Shelves Full` quest giver | Provisioner whose shop models food, bandages, lamp oil, and everyday survival pressure. |
| Halia Vey | Named faction agent | Delvers' Exchange, Act 2 claims council | `Silence Blackglass Well` and `Recover the Compact Waymap` quest giver; Act 2 Exchange sponsor | Polished guild agent who treats routes and claims as leverage. |
| Daran Orchard | Named veteran | Orchard Wall, Act 2 warden lane | `Break the Red Mesa Raiders`, old adventurer's cache scene, and woodland-saboteur quest source | Retired half-elf adventurer who reads Iron Hollow's roads like old battle lines. |
| Linene Ironward | Named merchant / quartermaster | Ironbound Trading Post, Act 2 claims council | Merchant, `Reopen the Trade Lane`, `Bring Back the Survey Team`, and `Hold the Claims Meeting Together` quest source | Disciplined quartermaster who values clean supply lines and proof over charm. |
| Mara Ashlamp | Named innkeeper / floorkeeper | Ashlamp Inn | Ashlamp regular, `The Marked Keg` quest giver, and barfight stabilizer | Runs the common room like triage: warm when possible, hard when needed, and never fooled for long by panic pretending to be fate. |
| Jerek Harl | Named grieving local | Ashlamp Inn | Ashlamp regular, `Bring Back Dain's Name` quest giver, grief dialogue source, and one of Sella's truth-detail speakers | Angry miner whose missing brother keeps the room from treating road deaths like weather until the player turns that grief into a real road-side answer. |
| Sella Quill | Named singer / rumor-collector | Ashlamp Inn | Ashlamp regular and `Songs for the Missing` quest giver | A traveling singer who hears more by making other people want to be quotable. |
| Old Tam Veller | Named retired prospector | Ashlamp Inn | Ashlamp regular, ruin-memory clue source, and one of Sella's truth-detail speakers | Rambling until he suddenly is not, especially when old stone, dead roads, or bad cellar air come up. |
| Nera Doss | Named courier | Ashlamp Inn | Ashlamp regular, `Quiet Table, Sharp Knives` quest giver, and one of Sella's truth-detail speakers | A battered courier who notices when messages start arriving half-rewritten by fear, money, or both. |
| Pale Witness | Named spirit / truth source | Hushfen and the Pale Circuit | Act 2 route lead tied to dead testimony and buried Meridian Compact truth | Treated as a warning source, not a standard combat enemy. |
| Ashlamp Teamster | Local witness group | Ashlamp Inn | Blackwake rumor and Sereth Vane callback speaker | Represents road-workers carrying fresh rumor and survivor testimony into Iron Hollow. |
| Town Council | Collective quest source | Iron Hollow, Resonant Vaults aftermath | `Sever the Quiet Choir` quest source | Represents the combined civic response once the mine threat becomes larger than a single sponsor. |
...
| Poisoned miner | Support patient | Lantern Shrine | Elira recruitment and `elira_helped` flag | Stabilizing the miner helps prove the player can be trusted with mercy and field care. |
| Rescued teamster / wounded witnesses | Support witnesses | Lantern vigil and Ashfall aftermath | Clue sources for Duskmere and Emberhall | Their testimony helps push Act 1 from Ashfall Watch toward the manor routes. |
...
| False roadwardens | Support antagonist group | Emberway False Roadwarden Checkpoint | Non-combat social pressure encounter; collapses under Deception, Insight, Persuasion, Intimidation, or Oren/Sabra/Garren proof | Borrowed-authority checkpoint hands who make the contract-house false-manifest thread matter before Blackwake. |
...
| --- | --- | --- | ---: | ---: | --- | --- |
| `goblin_skirmisher` | Goblin Skirmisher | Goblin Skirmisher | 1 | 6 / 13 | Prologues, Emberway, random scavenger fights | Cowardly nimble raider. |
| `wolf` | Ash Wolf | Beast Hunter | 1 | 11 / 13 | Prologue threats, Emberway pressure, road random encounters | Pack tactics and prone pressure. |
...
| `skeletal_sentry` | Skeletal Sentry | Undead Sentry | 1 | 13 / 12 | Blackglass Well and Duskmere undead guards | Site defender with undead fortitude flavor. |
| `worg` | Worg | Monstrosity Hunter | 1 | 18 / 13 | Red Mesa Hold and Ashfall support beast | Stronger pack hunter with prone rush. |
| `orc_raider` | Orc Raider | Orc Raider | 1 | 16 / 13 | Red Mesa Hold and Ashfall muscle | Aggressive melee humanoid. |
| `orc_bloodchief` | Orc Bloodchief | Orc Bloodchief | 2 | 33 / 15 | Red Mesa Hold boss base, usually Brughor Skullcleaver | Leader with `war_cry`. |
| `ogre_brute` | Ogre Brute | Ogre Brute | 2 | 38 / 10 | Red Mesa Hold heavy support | Big prone-causing hitter. |
| `gravecaller` | Gravecaller | Human Occultist | 2 | 26 / 13 | Factory-defined grave-salvage leader; legacy/backup for Blackglass Well style content | Fear and ash-veil caster. |
| `vaelith_marr` | Vaelith Marr | Human Occultist | 3 | 36 / 15 | Blackglass Well named boss | Upgraded gravecaller with ritual surge. |
| `nothic` | Nothic | Aberration Seer | 3 | 34 / 14 | Duskmere cistern horror, often named Cistern Eye | Weird insight and rotting gaze. |
...
| `mireweb_spider` | Mireweb Spider | Beast Hunter | 1 | 12 / 14 | Duskmere / cave-side optional support | Web-walker restrainer with venom web. |
...
| `rust_shell_scuttler` | Rust-Shell Scuttler | Monstrosity Scuttler | 1 | 14 / 13 | Blackglass Well and route hazard support | Corrosion nuisance. |
| `lantern_fen_wisp` | Lantern Fen Wisp | Undead Wisp | 2 | 16 / 16 | Blackglass Well boss support and eerie route hazards | Lure glow and vanish. |
| `ashstone_percher` | Ashstone Percher | Elemental Percher | 2 | 20 / 15 | Blackglass Well and Cinderfall ruin ambusher | False appearance, flyby, drop-strike style pressure. |
...
| `bugbear_reaver` | Bugbear Tunnel Reaver | Bugbear Reaver | 2 | 25 / 15 | Red Mesa Hold and cave ambush support | Surprise attack and abduct pressure. |
| `ettervine_webherd` | Ettervine Webherd | Monstrosity Webherd | 3 | 28 / 15 | Red Mesa Hold optional support | Spider-climb web controller with reel strand. |
| `carrion_lash_crawler` | Carrion Lash Crawler | Monstrosity Crawler | 3 | 30 / 15 | Blackglass Well and Stonehollow side pressure | Ceiling hunter with carrion tentacles. |
| `cache_mimic` | Cache Mimic | Monstrosity Mimic | 3 | 34 / 13 | Duskmere / treasure-trap support | Adhesive false treasure with greedy lure. |
...
| Vaelith Marr | Unique template `vaelith_marr` | Blackglass Well | Named gravecaller boss. |
| Brughor Skullcleaver | `orc_bloodchief` | Red Mesa Hold | Named orc blood-chief boss. |
...
| Sister Caldra Voss | Unique template `caldra_voss` | Meridian Forge | Act 2 final boss and Quiet Choir field theologian. |
...
| Tor Lookout | `orc_raider` | Enemy reference | Documented Red Mesa Hold named variant. |
...
| Cistern Eye | `nothic` | Duskmere nothic lair | Named cellar horror. |
...
| Blackglass / Wyvern / frontier bosses | Vaelith Marr, Brughor Skullcleaver, Cistern Eye |
