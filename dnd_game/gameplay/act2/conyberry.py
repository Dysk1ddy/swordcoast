...
class StoryAct2ConyberryMixin:
    ...
    def _hushfen_circuit_strain(self) -> int:
        assert self.state is not None
        value = self.state.flags.get("hushfen_circuit_strain", 0)
        return int(value) if isinstance(value, int) and not isinstance(value, bool) else 0

    def _hushfen_shift_circuit_strain(self, delta: int) -> int:
        assert self.state is not None
        current = self._hushfen_circuit_strain()
        updated = max(0, current + delta)
        self.state.flags["hushfen_circuit_strain"] = updated
        return updated

    def _hushfen_hushed_pilgrim_road(self, *, delayed: bool) -> None:
        assert self.state is not None
        self.say(
        ...
        if choice == 1:
            ...
            if self.skill_check(self.state.player, "Persuasion", 13, context="to steady the frightened road into Hushfen"):
                self.state.flags["hushfen_pilgrims_steadied"] = True
            ...
            if self.skill_check(self.state.player, "Insight", 13, context="to isolate the clean witness on the Hushfen road"):
                self.state.flags["hushfen_clean_witness_taken"] = True
            ...
            if self.skill_check(self.state.player, "Survival", 13, context="to track the wrongness moving through Hushfen's road"):
                self.state.flags["hushfen_whisper_track_named"] = True
    ...
    def _hushfen_waymarker_cairn(self) -> None:
        assert self.state is not None
        self.say(
        ...
        self.state.flags["hushfen_first_site"] = first_site
        if choice == 1:
            ...
            if self.skill_check(self.state.player, "Religion", 13, context="to read Hushfen's warded waymarker correctly"):
                self.state.flags["hushfen_cairn_ward_read"] = True
            ...
            if self.skill_check(self.state.player, "History", 13, context="to read Hushfen's grave-route order"):
                self.state.flags["hushfen_cairn_grave_read"] = True
            ...
            if self.skill_check(self.state.player, "Investigation", 13, context="to read the tampered Hushfen trail logic"):
                self.state.flags["hushfen_cairn_trail_read"] = True
    ...
    def _hushfen_chapel_of_lamps(self) -> None:
        assert self.state is not None
        self.state.flags["hushfen_chapel_seen"] = True
        self.say(
        ...
        if choice == 1:
            ...
            if self.skill_check(self.state.player, "Religion", 13, context="to relight Hushfen's chapel cleanly"):
                self.state.flags["hushfen_chapel_relit"] = True
                self._hushfen_shift_circuit_strain(-1)
            ...
            if self.skill_check(self.state.player, "Survival", 13, context="to take Hushfen's field lantern without stripping it thoughtlessly"):
                self.state.flags["hushfen_field_lantern_taken"] = True
                if elira is not None:
            ...
            if self.skill_check(self.state.player, "Investigation", 13, context="to quarantine the damaged chapel without worsening it"):
                self.state.flags["hushfen_chapel_quarantined"] = True
    ...
    def _hushfen_grave_ring(self) -> None:
        assert self.state is not None
        self.state.flags["hushfen_grave_seen"] = True
        self.say(
        ...
        if choice == 1:
            ...
            if self.skill_check(self.state.player, "History", 13, context="to read the grave ring as Meridian Compact route memory"):
                self.state.flags["hushfen_grave_history_read"] = True
            ...
            if self.skill_check(self.state.player, "Religion", 13, context="to name the dead cleanly enough for the ring to settle"):
                self.state.flags["hushfen_dead_named"] = True
                self._hushfen_shift_circuit_strain(-1)
            ...
            if self.skill_check(self.state.player, "Investigation", 13, context="to find the living claim marks hiding in the grave ring"):
                self.state.flags["hushfen_claim_marks_found"] = True
                self.state.flags["hushfen_claim_cover_suspected"] = True
    ...
    def _hushfen_defiled_sigil(self, *, delayed: bool) -> None:
        assert self.state is not None
        self.state.flags["hushfen_sigil_seen"] = True
        self.say(
        ...
        if choice == 1:
            ...
            if self.skill_check(self.state.player, "Religion", 13, context="to break the Choir's sigil without teaching it a second lesson"):
                self.state.flags["hushfen_sigil_broken"] = True
                self._hushfen_shift_circuit_strain(-1)
                if elira is not None:
                    self.speaker(
                        "Elira Lanternward",
            ...
            if self.skill_check(self.state.player, "Arcana", 13, context="to copy the defiled ward pattern before it collapses"):
                self.state.flags["hushfen_sigil_copied"] = True
                self._hushfen_shift_circuit_strain(1)
                if elira is not None:
                    self.speaker(
                        "Elira Lanternward",
            ...
            if self.skill_check(self.state.player, "Stealth", 13, context="to bait the sigil's watcher without letting them own the angle"):
                self.state.flags["hushfen_watcher_baited"] = True
                self.state.flags["hushfen_watcher_seen"] = True
                if bryn is not None:
                    self.speaker(
                        "Bryn Underbough",
    ...
    def _hushfen_visit_branch_site(self, site: str, *, delayed: bool) -> None:
        if site == "grave":
            self._hushfen_grave_ring()
        elif site == "sigil":
            self._hushfen_defiled_sigil(delayed=delayed)
        else:
            self._hushfen_chapel_of_lamps()

    def _hushfen_first_branch_site(self, *, delayed: bool) -> None:
        assert self.state is not None
        first_site = str(self.state.flags.get("hushfen_first_site", "chapel"))
        self._hushfen_visit_branch_site(first_site, delayed=delayed)

    def _hushfen_second_branch_site(self, *, delayed: bool) -> None:
        assert self.state is not None
        site_options: list[tuple[str, str]] = []
        if not self.state.flags.get("hushfen_chapel_seen"):
        ...
        if not self.state.flags.get("hushfen_grave_seen"):
        ...
        if not self.state.flags.get("hushfen_sigil_seen"):
        ...
        self.state.flags["hushfen_second_site"] = site
        self._hushfen_visit_branch_site(site, delayed=delayed)
        self.state.flags["hushfen_two_sites_answered"] = True

    def _hushfen_exit_warning_choice(self, *, success: bool, delayed: bool) -> None:
        assert self.state is not None
        if self.state.flags.get("hushfen_warning_exit_choice"):
            return
        if success:
            self.say(
        ...
        if choice == 1:
            ...
            self.state.flags["hushfen_warning_exit_choice"] = "public"
            self.state.flags["pale_witness_warning_shared_publicly"] = True
            self.state.flags["pale_witness_public_warning_known"] = True
            self.act2_shift_metric(
                "act2_town_stability",
                1,
            ...
            self.state.flags["pale_witness_warning_restricted"] = True
            self.act2_shift_metric(
                "act2_route_control",
                1,
            ...
            self.state.flags["hushfen_warning_exit_choice"] = "bound"
            self.state.flags["pale_witness_warning_bound"] = True
            self.state.flags["pale_witness_bound_leverage"] = True
            self.act2_shift_metric(
                "act2_route_control",
                1,
    ...
    def scene_hushfen_pale_circuit(self) -> None:
        assert self.state is not None
        delayed = self.state.flags.get("act2_neglected_lead") == "hushfen_truth_secured"
        if "hushfen_circuit_strain" not in self.state.flags:
            self.state.flags["hushfen_circuit_strain"] = 1 if delayed else 0
        self.banner("Hushfen and the Pale Circuit")
        self._hushfen_hushed_pilgrim_road(delayed=delayed)
        self._hushfen_waymarker_cairn()
        self._hushfen_first_branch_site(delayed=delayed)
        self._hushfen_second_branch_site(delayed=delayed)
        ...
        if delayed and self.state.flags.get("hushfen_circuit_defiled"):
            self.say(
        ...
        if self.state.flags.get("hushfen_pilgrims_steadied"):
            dc -= 1
        if self.state.flags.get("hushfen_dead_named") or self.state.flags.get("hushfen_chapel_relit"):
            dc -= 1
        if self.state.flags.get("hushfen_sigil_copied"):
            dc += 1
        self.run_dialogue_input("act2_hushfen_entry")
        choice = self.scenario_choice(
        ...
        if success and not delayed and strain <= 1:
            ...
            self.state.flags["pale_witness_truth_clear"] = True
            if choice == 1:
                self.state.flags["pale_witness_public_warning_known"] = True
            elif choice == 2:
                self.state.flags["pale_witness_pact_restraint_known"] = True
            ...
            self.state.flags["pale_witness_truth_clear"] = False
            if choice == 1:
                self.state.flags["pale_witness_public_warning_known"] = True
            elif choice == 2:
                self.state.flags["pale_witness_pact_restraint_known"] = True
        ...
        self._hushfen_exit_warning_choice(success=success, delayed=delayed)
        self.state.flags["hushfen_truth_secured"] = True
