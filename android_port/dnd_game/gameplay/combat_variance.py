...
def _near_miss_pressure(game: TextDnDGame, attacker, target, total: int, target_number: int) -> bool:
    return game.can_apply_pressure_result(attacker, target, total=total, target_number=target_number)
...
def simulate_weapon_action_once(game: TextDnDGame, attacker, target, rng: random.Random) -> ActionSample:
    ...
    accuracy = (
        ...
        + game.karmic_accuracy_bonus(attacker)
    )
    roll = rng.randint(1, 20)
    total = roll + accuracy
    pressure_was_available = game.has_status(target, "pressured")
    if pressure_was_available:
        game.clear_status(target, "pressured")
...
def simulate_arcane_bolt_once(game: TextDnDGame, actor, target, rng: random.Random, *, action_cast: bool = False) -> ActionSample:
    ...
    accuracy = (
        ...
        + game.target_accuracy_modifier(target)
        + game.karmic_accuracy_bonus(actor)
    )
    roll = rng.randint(1, 20)
    total = roll + accuracy
    pressure_was_available = game.has_status(target, "pressured")
...
def _apply_pressure_statuses(target, sample: ActionSample) -> None:
    if sample.result_kind == "pressure" and getattr(target, "is_conscious", lambda: False)():
        target.conditions["pressured"] = 99


def _record_sample_result(game: TextDnDGame, actor, sample: ActionSample) -> None:
    if sample.result_kind == "cooldown":
        return
    game.record_karmic_hostile_action_result(actor, meaningful=sample.result_kind in {"pressure", "glance", "wound"})
...
def simulate_simple_encounter_once(enemy_factories: tuple[Callable[[], object], ...], seed: int, *, max_rounds: int = 12) -> tuple[bool, int, int]:
    ...
    while round_number < max_rounds and any(enemy.is_conscious() for enemy in enemies) and any(member.is_conscious() for member in party):
        ...
        for actor in party:
            ...
            _record_sample_result(game, actor, sample)
        ...
        for enemy in enemies:
            ...
            _record_sample_result(game, enemy, sample)
            _apply_pressure_statuses(target, sample)
            _apply_sample_damage(target, sample)
